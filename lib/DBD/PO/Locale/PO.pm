...
sub new {
    ...
    $self->obsolete( $options{'-obsolete'} )
        if defined( $options{'-obsolete'} );
    $self->c_format(1)
    ...
    while (<$file_handle>) {
        ...
        if (/^$/) {
            ...
            if ( defined( $po->automatic ) ) {
                $po->automatic( $po->automatic . "$eol$1" );
            }
            else {
                $po->automatic($1);
            }
            ...
            if ( defined( $po->reference ) ) {
                $po->reference( $po->reference . "$eol$1" );
            }
            else {
                $po->reference($1);
            }
